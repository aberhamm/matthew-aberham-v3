  ...
  return (
    <ComponentWrapper className="h-[450px]">
        ...
        >
          {IMAGES.map((image, index) => (
            <motion.img
              ...
              className={cn(
                'h-[200px] w-[200px] min-w-[200px] rounded-sm select-none',
      ...
      >
        {IMAGES.map((_, index) => (
          <motion.div
            ...
            onClick={() => setFocusedIndex(index)}
            className={cn(
              'bg-muted/60 w-2.5 cursor-pointer rounded-full transition-colors',
      ...
      <div className="absolute top-2 left-2 flex gap-2">
        <button
          className="inline-flex h-8 w-8 items-center justify-center transition-transform active:scale-95"
          onClick={() => {
