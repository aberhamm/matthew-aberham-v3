...
function FilterItem({
  ...
  return (
    <motion.button
      type="button"
      className={cn(
        'text-secondary-foreground ml-3 inline-flex h-8 cursor-pointer items-center justify-center rounded-full border px-2 text-sm',
        active ? 'bg-secondary' : 'bg-background text-muted-foreground',
        active &&
          isBound === 'single' &&
          'bg-secondary text-secondary-foreground border-muted dark:border-muted brightness-[.97]',
        className
      )}
      variants={variants}
      animate={isBound}
