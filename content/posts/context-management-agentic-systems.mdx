...
- **File reads** truncate at line boundaries and return pagination hints pointing the agent to the next region (for example, an `offset=500` suggestion appended to the truncated output). Claude Code reads 2,000 lines per call by default with explicit start/limit params. OpenCode rejects files over 250KB outright with a message telling the agent to use a narrower approach. SWE-agent uses a 100-line sliding window that the agent scrolls through with `scroll_up` and `scroll_down` commands.
